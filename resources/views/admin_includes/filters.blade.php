    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-4">
                            @if ($data['type'] != 'select' && $data['type'] != 'date')
                                ...
                                <input type="{{ $data['type'] }}" value="{{ request()->get($column_name) }}" name="relations[{{ $data['related_to'] }}][{{ $column_name }}]" class="form-control" placeholder="Type here">
                                @else
                                <input type="{{ $data['type'] }}" value="{{ request()->get($column_name) }}" name="{{ $column_name }}" class="form-control" placeholder="Type here">
                                @endisset
                            @elseif($data['type'] == 'select')
                                {!! Form::label($column_name, Str::ucfirst($data['label']), ['class' => 'font-weight-bold']) !!}
                                <select name="{{ $column_name }}" id="{{ $column_name }}" class="form-control" {{ $column_name == 'trainer_id' && Auth()->user()->roles[0]->title == 'Trainer' ? 'readonly' : '' }}>
                                    <option disabled selected>{{ $data['label'] }}</option>
                                    @foreach ($data['data'] as $id => $col)
                                        <option {{ request()->get($column_name) == $id ? 'selected' : '' }} value="{{ $id }}">{{ $col }}</option>
                                    @endforeach
                                </select>
                            @endif
                        </div>
            ...
            <div class="modal-footer">
                ...
                <button type="reset" class="btn btn-info" id="resetBtn"><i class="fa fa-refresh"></i> Re-filter</button>
                <button type="button" class="btn btn-danger" data-dismiss="modal"> <i class="fa fa-times"></i> {{ trans('global.cancel') }} </button>
                @isset($columns)
                <button type="submit" class="btn btn-primary"> <i class="fa fa-check"></i> {{ trans('global.filter') }}</button>
                @endisset
            </div>
            {!! Form::close() !!}
        </div>
